...
export class GameServer {
  ...
  private moveSnake(player: Player): void {
    ...
    // World wrapping
    // 改成：邊界偵測（撞牆即死）
if (newHead.x < 0 || newHead.x > this.worldSize.width || 
    newHead.y < 0 || newHead.y > this.worldSize.height) {
  
  player.snake.isAlive = false; // 讓蛇死掉
  this.dropSnakeAsFood(player.snake); // 讓蛇原地變成食物
  console.log(`💀 ${player.name} 撞牆了！`);
  return; // 結束這次移動，不要再畫頭了
}
